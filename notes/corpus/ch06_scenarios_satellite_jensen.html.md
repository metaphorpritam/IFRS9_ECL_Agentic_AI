# ch06_scenarios_satellite_jensen.html

Ch.6 — Scenarios, Satellite Models & Jensen's Inequality | IFRS9 ECL Study Notes

☼

# Chapter 6 — Scenarios, Satellite Models & Jensen's Inequality

From DFAST supervisory paths through a fitted satellite model to a proof that probability-weighted ECL exceeds single-path ECL

IFRS9 ECL Study-Notes Compendium — Chapter 6 of 13. Compiled from 
...
, 
outputs/scenario_ecl/scenario_ecl_report.md
, and 
...
## 6  Scenarios, Satellite Models & Jensen's Inequality

Chapter 5 built the machine that turns a systematic factor $Z$ into a point-in-time PD — but never said
where $Z$ comes from when you are trying to look 
forward
 rather than recover it from history. IFRS 9
§5.5.17(c) requires ECL to reflect 
reasonable and supportable
 information about future conditions,
and §5.5.18/B5.5.42 goes further: it must be an 
unbiased, probability-weighted
 estimate
evaluated across a 
range
 of outcomes, not the single most-likely path. This chapter builds the whole
...
outcomes
 is not the same as evaluating the outcome of the averaged scenario. That gap has a name —
...
knowledge/sources/ifrs9_credit_risk_notes.md
 §9 (all sections); project evidence from
...
, and the
three 
outputs/{scenarios,satellite,scenario_ecl}/
 reports.
...
Scenario
 — a full, internally-coherent macro path (unemployment, house prices, GDP, rates
all moving together sensibly) over some horizon, standing in for one possible future state of the economy.

Probability weight
 $w_s$ — the bank's own governance-committee judgment of how likely
...
Probability-weighted ECL
 — $ECL_{PW} = \sum_s w_s \cdot ECL(s)$: run the full ECL engine
once 
per scenario
, then average the resulting ECL figures.

Single-path ECL
 — $ECL(\bar{s})$: average the macro 
paths
...
Theorem (quoted, §9.2) — Jensen's inequality applied to ECL.
 ECL is a convex function of the macro
state. For convex $f$, $\mathbb{E}[f(X)]\ge f(\mathbb{E}[X])$. Hence $ECL_{PW} \ge ECL(\bar{s})$ — measuring on
the single most-likely path systematically 
understates
 expected loss. This is the analytical reason
...
the full machinery needed to compute both correctly: §6.2–6.4 build the scenario 
paths

themselves (where do three internally-coherent 40-quarter macro paths even come from), §6.5–6.6 build
the 
satellite model
 that turns a macro path into a $Z$ path, and §6.7–6.9 close the loop with
the Jensen's-inequality proof and its two concrete numeric instances.

Gotcha — "probability-weighted" does not mean "statistically estimated".
 There is no dataset of "how
...
rationale
, not the third decimal place of the weight.
...
The project's answer: ingest the shapes a supervisor already designed. 
DFAST
 (the Dodd-Frank
Act Stress Test) publishes 28-variable macro paths built for exactly this kind of coherence check —

data/ingest/dfast.py
 parses the 2026 vintage's Baseline and Severely-Adverse 13-quarter CSVs
...
Source: 
outputs/scenarios/scenarios_report.md
 "DFAST column -> panel concept ->
transformation"; 
data/ingest/dfast.py
.

Gotcha — SAAR is an ANNUALISED rate, not a quarterly one.
 DFAST publishes GDP growth as a
...
levels
 onto the panel would create a nonsensical jump (unemployment does not teleport from a 2015
value to whatever the 2026 print happens to be) purely as an artefact of two unrelated calendars meeting. DFAST's
real value is not its absolute levels in 2026 — it is the 
coherent multivariate shape
 a
...
Take the DFAST path as a sequence of DELTAS from that
jump-off,
 not as levels: $\delta_h(c) = \text{dfast}_h(c) - \text{dfast\_jumpoff}(c)$ for horizon
...
Add that delta onto the PANEL's own $t=60$ level,
...
This is a shape transplant, not a forecast made in
2015.
 The resulting path is not "what the panel's macro variables were predicted to do in 2015" (DFAST
...
level. 
engine/scenarios.py
's own docstring is explicit about this framing.

Worked example — one variable, one quarter, every number shown.
 National unemployment, DFAST
Severely-Adverse, 2026Q1 (
data/ingest/dfast.py
, 
engine/scenarios.py
, re-run and
...
7.10pp
computed, matches 
engine.scenarios.build_scenario_set()
's own 
down
 path, $h=1$ (calendar 2015Q2)
...
jump-off)" figure 
outputs/scenarios/scenarios_report.md
 reports. Both rows use the identical
2-step delta-then-rebase mechanism; only the DFAST quarter and level change.

concept
panel jump-off, $t{=}60$
panel long-run mean (reversion target, §6.4)
...
Source: 
outputs/scenarios/scenarios_report.md
 "Rebasing convention" table.

Interpretation — why deltas, never levels.
 The rebasing convention deliberately throws away everything
...
Do not read the rebased path as "the panel's own 2015 forecast".
 The panel's $t=60$ reporting date is a
...
The damped-mirror convention.
 $$ \delta^{up}_h(c) \;=\; \kappa \cdot \delta^{down}_h(c), \qquad
...
boom, not an implausible one. All four figures re-derived directly from 
engine.scenarios.build_scenario_set()
.

Interpretation — why mirror the SEVERE path rather than the baseline, and why damp it.
 Mirroring the
...
Governance framing — an honestly-labelled placeholder, with its upgrade path named.

outputs/scenarios/scenarios_report.md
 and 
engine/scenarios.py
's docstring both flag
...
Gotcha — a mirrored path is not a symmetric RISK statement.
 It is tempting to read the up/down pair as
...
The extension convention.
 R&S window $=13$ quarters ($h=1..13$, the DFAST length); reversion ramp
...
4.
 From $h=21$ to $h=40$ the value simply HOLDS at $\bar{v}$ — no
further computation, the path is flat for the remaining 19 quarters.

Interpretation.
 The reversion ramp is a straight line in concept-space, not in PD- or ECL-space — it is
...
Exhibit 6.1
 — The full scenario-conditional ECL pipeline this chapter builds,
DFAST CSV to probability-weighted allowance. Regenerated in matplotlib
(
notes/assets/img/ch06/build_diagrams.py
, numbers from 
outputs/scenario_ecl/scenario_ecl_report.md
).

Exhibit 6.2
 — Scenario $Z$ paths from the satellite (§6.5): the recovered
...
, 
outputs/scenario_ecl/scenario_z_paths.csv
).
...
app/api/main.py
) — the notes and the app cannot silently disagree on what these terms mean.

Variable card — 
hpi_growth_lag1
 (National HPI, log-growth, 1-quarter lag).

Source.
 The DCR panel's own national House Price Index series. 
Honesty check:

this is 
NOT
 a live FRED pull — the DCR panel's macro columns (UER, HPI, GDP) are a
vendor-premerged national series on the panel's own ANONYMIZED quarterly clock
(
data/panel/build_panel.py
: "macros pre-merged"). The clock's CALENDAR alignment to real quarters
...
Transformation + lag rationale.
 $hpi\_growth = \Delta\log(HPI)$, a one-quarter LOG
...
Units and scale — the log-growth trap.
 One "unit" of $hpi\_growth$ (as fed to the
...
Economic channel.
 Collateral / negative-equity channel: rising house prices rebuild
...
Variable card — 
gdp_growth_lag2
 (National real GDP growth, 2-quarter lag).

Source.
 Same DCR panel, national real GDP growth series — same honesty statement as the HPI
...
Transformation + lag rationale.
 A plain quarterly growth RATE (not log-differenced, unlike
...
Units and scale.
 One "unit" of $gdp\_growth$ here is ALREADY a whole percentage point (a
...
Economic channel.
 Labour-income / general economic-activity channel: GDP growth is the
...
Source: 
outputs/satellite/satellite_report.md
 "Stationarity hygiene". KPSS $p$-values
are table-bounded to $[0.01,0.10]$.

Reading the table plainly.
 The unemployment LEVEL is the one clear-cut I(1) case (ADF cannot reject a
...
Specification search — AIC over 26 candidate specs, under a sign-governance constraint.
 Every non-empty
...
$hpi\_growth\_lag1$ + $gdp\_growth\_lag2$ 
(chosen)
117.05
0.466
1.39
...
$duer\_lag1$ + $hpi\_growth\_lag1$ + $gdp\_growth\_lag1$
117.38
0.472
1.56
no

$duer\_lag2$ + $hpi\_growth\_lag1$ + $gdp\_growth\_lag2$
117.54
0.470
1.46
no

$hpi\_growth\_lag2$ + $gdp\_growth\_lag2$
117.86
0.459
1.44
yes

$hpi\_growth\_lag1$ + $gdp\_growth\_lag1$
118.21
0.455
1.46
yes

… 21 further specs (full grid, nothing hidden)
…
…
…
…

$gdp\_growth\_lag1$ (driver alone)
133.12
0.280
1.00
yes

$duer\_lag2$ (driver alone)
148.49
0.058
0.71
yes
...
Exhibit 6.3
 — The satellite fit: recovered $Z_t$ vs the two-driver fitted curve,
residuals below, GFC shaded, real calendar quarters throughout. Already in the project's own house matplotlib
style — embedded, not redrawn (
outputs/satellite/satellite_fit.png
).

Reading the exhibit.
 The fitted curve (orange dashed) tracks the recovered $Z_t$'s (purple solid) broad
...
Simplified vs full method — stated plainly, not glossed over.
 This is a STATIC OLS in already-transformed
(I(0)) drivers with AIC lag selection, sign governance and a GFC-dummy sensitivity check. The PRODUCTION
STANDARD (§9.1, and named explicitly in 
outputs/satellite/satellite_report.md
) is: ARDL bounds
...
1.
 $Z_t$ is an EXPLICIT function of exactly two inputs:
...
2.
 Suppose a UER shock is applied "naively": add $+2$pp to the
scenario path's $uer$ column, hold $hpi\_growth$ and $gdp\_growth$ at their unshocked (base-scenario) values.

3.
 Evaluate $Z_t$ on the shocked path: since neither
...
4.
 Since $Z$ is unchanged, $PD_{PIT}(Z)$ (Chapter 5) is
...
The coherent-shock convention.
 Define the co-movement loading of concept $c$ onto a $1$-unit move of
...
Worked example — the full chain, one shock, every number shown.
 Shock: UER $+2$pp, shape

parallel
. All values re-derived directly from a live re-run of 
agent/tools_tier1.py
's

shock_macro
 tool against the project's own frozen engine (warm model cache,

outputs/models/tier1_models.joblib
) — this reproduces the recorded trace in
...
Co-movement loadings
 (severe-minus-base,
13-quarter R&S window; 
agent.tools_tier1._comovement_betas
):
...
Per-concept deltas
, applied at EVERY quarter
(shape 
parallel
 $\Rightarrow \text{profile}(h)=1$ for all $h$), with $s=+2.0$pp:
...
Propagation into the shocked $Z$ path.
 Because
...
Vasicek PIT-condition and re-run the frozen ECL
engine
 ($\rho=0.0227$, Chapter 5's own calibration) on all 7,849 live/performing exposures at the
...
Matching, to display rounding, the recorded agent trace: 
"UER +2pp (parallel) coherent shock of the base
...
Exhibit 6.4
 — The coherent-shock convention: a naive univariate UER shock (left, red)
...
Interpretation — this is the agent's Tier-1 
shock_macro
 tool, not a one-off calculation.
...
's 
shock_macro
) standing between a natural-language macro
...
Gotcha — a missing regressor does not mean "the tool ignores your shock".
...
No — 
peak_revert
 ramps the shock up over 4 quarters, holds it through the
  ...
  $-0.19228$ steady state in the worked example is specific to the 
parallel
 shape, which is a
...
Derivation.
 Let $x_{1,h,s}, x_{2,h,s}$ denote scenario $s$'s $hpi\_growth\_lag1$, $gdp\_growth\_lag2$ at
horizon $h$, and $Z_{h,s}=c+b_1 x_{1,h,s}+b_2 x_{2,h,s}$ the satellite (§6.5), with weights $w_s$,
$\sum_s w_s=1$.

1.
 The weighted-AVERAGE macro path's own driver values at $h$ are,
by definition, $\bar x_{1,h}=\sum_s w_s x_{1,h,s}$ and $\bar x_{2,h}=\sum_s w_s x_{2,h,s}$ (§6.1's
$\overline{path}$ definition).

2.
 Evaluate the satellite AT that averaged path:
$Z(\bar{path})_h = c+b_1\bar x_{1,h}+b_2\bar x_{2,h}$.

3.
 Compare against the weighted average of the PER-SCENARIO $Z$
...
4.
 Steps 2 and 3 are the SAME expression: $$ Z(\bar{path})_h =
...
Interpretation.
 This lemma is why §6.1's two definitions of "single-path ECL" — "average the macro
...
Convexity.
 A twice-differentiable function $f$ is CONVEX on an interval if $f''(x)\ge0$ for every $x$ in
...
5.
 Substituting step 4 into step 3: $$ \boxed{\,
...
Interpretation.
 The whole proof is one idea: a convex curve sits above every one of its own tangent
...
Second derivative, via the chain rule again.
...
Worked example — locating the inflection point, both calibrations.
 Numerically verified against a
...
Interpretation — why the inflection point is always this far out.
 $c=\Phi^{-1}(PD_{TTC})$ is strongly
...
1.
 $ECL(Z) = EAD\cdot LGD\cdot PD_{PIT}(Z)$ (Chapter 2's ECL
...
2.
 $EAD\cdot LGD$ is a POSITIVE constant with respect to $Z$
...
3.
 Since $EAD\cdot LGD>0$, the sign of $d^2 ECL/dZ^2$ is IDENTICAL
...
Putting it together.
 By part (c), $ECL(Z)$ is convex over the realistic $Z$ range. By part (a),
...
Gotcha — the proof needs convexity over the RANGE actually visited, not everywhere.
 Jensen's inequality
...
's 
RESULTS
 (which reproduces the source notes'
own printed §9.2 table to displayed precision).

The toy setup (§9.2's worked example).
 $EAD=$€$100$m, $LGD=40\%$, $PD_{TTC}=2\%$, $\rho=0.12$
...
Weighted-average GDP growth path:

$$ \bar g = 0.25(3.5)+0.50(2.0)+0.25(-2.5) = 0.875+1.0-0.625 = 1.25\%. $$

3.
 
$Z$ and $PD_{PIT}$ AT that averaged path
 (the
...
Single-path ECL
 (§6.1's $ECL(\bar s)$):
$$ ECL(\bar s) = 0.022499\times0.40\times100 = 0.9000\text{m} \to \boldsymbol{\text{\euro}0.90\text{m}}. $$

5.
 
The Jensen gap, quantified:
...
Both figures match 
RESULTS['understatement_pct']
 ($48.15$, notes' own printed "$48\%$") and

RESULTS['weighted_over_single_ratio']
 ($1.9286$, notes' own printed "$\sim1.9\times$").

Exhibit 6.5
 — The toy fixture's Jensen gap, drawn from the real closed-form
...
Reading the exhibit.
 The three scenario dots sit EXACTLY on the curve — unlike §6.9's real-book
...
The Jensen gap, on the live book.
 Reported allowance $ECL_{PW}=\$34.0$m vs $ECL(\bar s)=\$32.9$m:
...
Exhibit 6.6
 — THE Jensen-gap exhibit on the live book: weighted allowance
...
Exhibit 6.7
 — Reported allowance by scenario, t=60 (2015Q1) live book, 7,849
...
Calibrated $\rho=0.0227$ vs the toy's $0.12$.
 Chapter 5 §5.7 already established
...
Scenario $Z$ dispersion.
 The satellite-implied paths touch their extremes for only a SINGLE
...
Only the default-PD leg is conditioned.
 §6.6 already flagged this: LGD and EAD stay at
...
Lifetime aggregation dilutes further.
 All three scenarios share the SAME long-run reversion
...
The lifetime up-vs-base crossover — reported, not hidden.
 Summed LIFETIME ECL is $289.6$ (upside) vs
$282.9$ (base) — the upside book is slightly MORE expensive over the full remaining life, even though its 12m
ECL and reported allowance are (correctly) the cheapest. Two real, documented mechanisms: (1) 
mirror
payback
 — the upside path is a damped MIRROR of the severely-adverse deltas (§6.3), so the
severe path's recovery quarters mirror into a mild LATE-window deterioration for the upside (boom now, payback
later), pulling its mean $Z$ over 21 quarters marginally below the base path's; (2) 
survivor
exposure
 — lower hazards in the boom leave MORE loans alive entering the shared long-run tail, so more
...
Interpretation.
 The swing across two reasonable alternative weightings is small — about $\pm2.1\%$ on
...
Do not read the smaller real-book ratio as "Jensen's inequality barely matters here".
 $1.035\times$ on
...
exactly 1 (the same tolerance-then-renormalise convention 
agent/tools_tier1.py
's

reweight_scenarios
 tool uses). At the defaults ($w_{up}=0.25$, $w_{down}=0.25$, so
...
What this widget is (and is not).
 This is a PORTFOLIO-LEVEL illustration using the flat composition-
...
Chapter 6 summary.
 DFAST supervisory paths give this project a coherent multivariate stress SHAPE for
