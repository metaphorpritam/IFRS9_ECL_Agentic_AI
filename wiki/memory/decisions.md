...
## 2026-07-08 11:08 UTC — tags: product, vision, north-star

PRODUCT NORTH STAR (operator, 2026-07-08 — binding on every future app/agent decision): the app is a CONSULTANT'S DELIVERABLE + CLIENT'S LAB. The consultant pre-generates the analysis; the client browses specifics in their own time, runs scenario experiments, and an LLM agent provides informed, data-grounded interpretation and INTUITION — never hallucinated numbers (Tier-1/2 compute, Tier-3 cite, narrator grounded). Implications accepted: (a) App v2 with tabs: Executive Overview / The Model (coefficients + interpretation + variable dictionary) / Scenario Lab (results + agent auto-interpretation after every run) / Policy (each exhibit paired with the decision it informs: SICR threshold dial, weights judgment, overlay playbook) / Copilot (agent front-and-centre: chat + live trace + audit browser); persistent mini-chat dock on every tab once Tier-2/3 land. (b) UI/UX quality pass required - current dashboard judged disappointing. (c) After every scenario run the agent auto-narrates grounded interpretation. (d) Workflow economics: executor/advisor model tiering - Sonnet-class authors, Fable-class only for adversarial review of high-stakes math.

## 2026-07-17 03:20 UTC — tags: freddie, rung3, data

SFLLD rung-3 conventions (Phase A, review-verified): default = D90 ABSORBING (first 90+DPD or RA; panel truncated after event month even if raw tape cures - unit-tested on a real curing loan); same-row D90/terminal tie-break: disposition code wins (~0.1-0.2% loans); liquidation + realized-loss fields kept on loan_orig.parquet from the UN-truncated tape; sentinels 9999/999/99/'9' -> NaN each documented; state macro from FRED ({POSTAL}UR monthly, {POSTAL}STHPI quarterly) with national fallback for GU/VI(+PR for HPI); no-lookahead HPI monthly fill; ISOLATION LAW: everything in freddie/ namespace, DCR frozen five NONE + panel.parquet sha-pinned at every gate. data/SFLLD/ added to .gitignore (licensed raw zips, 600MB).

## 2026-07-18 20:47 UTC — tags: freddie, phaseB, covid

SFLLD Phase-B decisions (review-verified): COVID regime = EXCLUDE window 2020-04..2021-09 for structural/scenario-conditional use (Fable review OVERTURNED the author's additive-dummy recommendation - the dummy did not repair the macro coefficient block; forbearance handled as scoring overlay); WESML nominal SEs shipped with documented caveat (seed-pair swing up to 5.7x nominal SE, seed_stability.csv is the honest uncertainty); LGD excess loading 0.0148 vs DCR 0.0255 with cycle-dependent severity dominating; backtest realized outcomes timed by first-D90 month from the truncated panel (NOT disposition month - critical review fix); LSTM stays challenger (lift is entirely delinquency-state memory: +0.387 AUC on prior-spell loans, near-random on clean history).
