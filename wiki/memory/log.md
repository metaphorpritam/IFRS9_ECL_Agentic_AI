...
## 2026-07-24 23:05 UTC — tags: cv, dossier-v2, ship

DOSSIER V2 + PROPAGATION SHIPPED (all 3 stages, no-ultracode mode: one Sonnet author agent, orchestrator reviewed personally). Stage 1: notes/cv_dossier.html rebuilt (1703 lines) — byte-exact 4-pointer card (programmatic diff PASS), S1-S6 defense sections each ending in a revalidation manifest with facts RE-EXECUTED at build time (665/548/133 counts, all fixture values, hazard 0.7476/0.6609 + prepay 0.6839/0.5841 + LGD/staging/ECL rescored from tier1_models.joblib; Vasicek + Jensen honestly labeled read-from-artifact), LangGraph/PageIndex/CI-CD deep-dives package-verified (langgraph 1.2.8, langchain-core 1.4.8, langchain NOT installed, zero bare imports), 12-Q gauntlet, PART III Freddie second act (15 Q&A + artifact index). Review catches: 03_rag_pipeline.png bottom box clipped at canvas edge (ylim 0 -> -0.35), 04_cicd_flowchart.png text overflowing the billing-locked box (rewrapped 3 lines) — agent's own image QA missed both; gates rerun by reviewer, PASS/0 defects. Stage 2 (direct edits, commit 64d4048): app Model tab "Model at a glance" (ECL sum, per-term jump chips + explain icons) + "EAD & EIR method" panel (annuity closed form, double-counting rule, CCF 14.0m, origination-EIR rationale, Stage-3 rule; grounded in engine docstrings), Copilot "How grounding works" (4-step guard chain + dot legend + honest attribution limitation; accuracy fix: copy states 4 Tier-1 + analyze_data + query_model_docs, not "four tools"), header Notes link; landing hero CV-dossier link + reading paths + model-components quick-index (deep ch01-ch08 anchors). Stage 3: main pushed (64d4048, a9a8e8c), Space app/ui/src uploaded (commit 39ddf96, RUNNING_BUILDING — old app live during build, bundle-grep monitor running), gh-pages rebuilt (0b978c0, corpus/ deliberately excluded) + explicit POST /pages/builds -> built; LIVE VERIFIED: card Problem row + "133 golden fixtures &amp; 548 tests" + landing "Model components" strip + new diagrams all serving on metaphorpritam.github.io. Housekeeping still open: rotate 4 keys, sudo passwd, GH Actions billing.

## 2026-07-25 00:20 UTC — tags: cv, dossier-v2, adversarial-review, mobile

ADVERSARIAL UI/UX REVIEW (user-prompted — the ship pass had been verification-only on UI): Playwright interaction + 390px + dark-mode audit. PASSES: glance chips scroll to correct panels (3/3 in-view), per-term + panel explain strips render THINKING and the graceful HTTP-429 single-worker message, app mobile zero overflow (Model + Copilot), dossier TOC present + anchors resolve, dossier/landing dark mode clean. DEFECTS FOUND+FIXED: page-level horizontal overflow at phone width — cv_dossier 232px (CV card table clipped, worst case), index 131px, ch03 186px (pre-existing sitewide), ch08 23/ch09 6/ch10 38 residuals (router-tree summary, pdoc dd, gotcha long tokens — found by display-toggle bisection). Root causes: (1) zero table/inline-code containment anywhere -> NEW notes/assets/apply_mobile_overflow_fix.py, idempotent <=700px-scoped injection into all 15 pages (tables+pre scroll in-box, long tokens wrap, .widget scrolls as unit); (2) cv_dossier.html NEVER had the math-overflow fix (its E[LGD] display math = the last 64px) -> added to apply_math_overflow_fix.py default targets. Post-fix: 15/15 pages zero overflow, check_notes 15/15 PASS, verify_math 0 defects. Shipped: main e2a5e80, gh-pages 84c63ce, Pages build built, live page serves both fix blocks. Lesson recorded: "review the ship" must include interaction/mobile/dark passes, not just desktop screenshots — the desktop-only pass missed the card table being unreadable on phones.
